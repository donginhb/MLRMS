...
namespace BEMAttendance.Controllers
{
    public class HeatPumpController : Controller
    {
        ...
        public ActionResult Index(string beginTime1, string endTime1, string searchkey ,string dptName1List, string index)
        {
            ...
            {
                beginTime1 = dt.AddMonths(-1).ToString("yyyy/MM/01 00:00");
                endTime1 = dt.AddMonths(-1).ToString("yyyy/MM/dd 23:59");
            }
            ...
            List<HeatPumpParas> attendanceResults = new List<HeatPumpParas>();
            try
            {
                SelectList dptSelectlist1 = null;
                using (mlrmsEntities db = new mlrmsEntities())
                {
                    var dptList = db.device.Where(m=>m.type==2).Select(m => new { slaveid = m.slaveid, devname = m.name }).ToList();
        ...
        public List<HeatPumpParas> GetPageAttendanceInfos(int index, string beginTime1, string endTime1, string searchkey,string slaveCode)
        {
            ...
            try
            {
                using (mlrmsEntities db = new mlrmsEntities())
                {
                    ...
                    if (slaveCode != "0")
                    {
                        if (index == 1)
                        {
                            totalCount = (from heatPump in db.device
        ...
        public FileResult Export(string beginTime1, string endTime1,string searchkey,string slaveid)
        {
            ...
            List<HeatPumpParas> list = GetAllAttendanceInfos(beginTime1,endTime1,searchkey, slaveid);
            ITransferData transdata = TransferDataFactory.GetUtil(DataFileType.CSV);
            DataTable dataTable = new HeatPumpInfoGetter().GetTable(list);
            ...
            return File(bytesConvent, "application/vnd.ms-excel", Server.UrlEncode(string.Format("热泵能耗信息{0}_{1}.csv",DateTime.Parse(beginTime1).ToString("yyyy-MM-dd-HH-mm-ss"),DateTime.Parse(endTime1).ToString("yyyy-MM-dd-HH-mm-ss"))));
